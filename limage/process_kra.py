...
class KRARoot(KRABase):
    def __init__(self, filepath):
        ...
        print("zip closed", self.name, self.width, self.height)
...
class KRALayer(KRABase):
    def __init__(self, data, root, parent):
        ...
        self.parent = parent
        self.layers = []
        self.x = int(data.attrib["x"])
        self.y = int(data.attrib["y"])
    ...
    def get_bounds(self):
    	return self.get_image_data().getbbox()
    ...
    def get_image_data(self):
        ...
        with zipfile.ZipFile(self.root.filepath, "r") as kra_file:
            ...
            if tile_count == 0:
                return Image.fromarray(numpy.zeros((1,1,4), dtype=numpy.uint8), "RGBA")
            ...
            self.bbox = (minx+bbox[0],miny+bbox[1],bbox[2],bbox[3])
            return image
...
def process(path, data:dict):
	...
	layers = [x for x in file.layers_recursive()]
